...
function showDaySchedule(dateKey) {
    ...
    if (list.length === 0) {
        ...
        dayList.innerHTML = list.map(s => {
            const icon  = s.type === 'ADOPTION'  ? 'event_available' : 'volunteer_activism';
            const color = s.type === 'ADOPTION'  ? '#e56b2e'         : '#0f9d7a';
            const label = s.type === 'ADOPTION'  ? '입양 상담'       : '봉사 활동';
            return `
                <div class="cal-schedule-item" style="border-left:3px solid ${color};">
                    ...
                    <span class="tag" style="margin-top:6px;background:#ffe7d8;color:#8f340b;">${label}</span>
                </div>`;
        }).join('');
    }
	...
	function renderFeed() {
		...
		container.innerHTML = pageItems.map(item => {
		    ...
		    const urlMap = {
		        ...
		        'VOLUNTEER': ctp + '/admin/volunteer/manage.do?volunteerId=' + item.id,
		        'RESCUE'   : ctp + '/admin/report/list.do?animalId=' + item.id,
