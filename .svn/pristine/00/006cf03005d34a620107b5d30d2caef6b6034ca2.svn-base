				...
				<div class="admin-card admin-card--wide clickable-card"
				    ...
				    <div class="adoption-status-container" style="display: flex; align-items: center; justify-content: space-between;">
				       ...
				       <%-- 입양 진행 현황 계산 로직 시작 --%>
				        ...
				        <%-- 원형 차트: adoptRate를 정수로 잘라서 CSS 변수와 텍스트에 사용 --%>
						<%-- adoptRate가 소수점 포함된 값이므로 정수 문자열로 변환 --%>
						<c:set var="adoptRateInt" value="${fn:substringBefore(adoptRate, '.')}" />
						<c:if test="${empty adoptRateInt}"><c:set var="adoptRateInt" value="0" /></c:if>
						
						<div class="chart-circle-wrapper">
						    <div class="chart-circle" style="--percent: ${adoptRateInt}%">
					    ...
					    <div class="schedule-list">
					        ...
					        <c:forEach var="s" items="${scheduleList}">
					            ...
					            <div class="schedule-item clickable-card"
					                ...
					                <div class="schedule-item__time">${s.TIME}</div>
